...
const hasDatabaseConfig = Boolean(rawConnectionString || hasSeparateConfig);
...
const createExecutor = (executor) => async (query, params = []) => {
  if (!executor) {
    throw new Error('DATABASE_URL Supabase PostgreSQL belum diisi di .env');
  }
...
if (!hasDatabaseConfig) {
  console.error('Database configuration missing: isi DATABASE_URL Supabase PostgreSQL di .env');
} else {
