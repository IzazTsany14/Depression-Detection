...
export const BKStudentCases: React.FC = () => {
  const { user, getAllStudents, getAllTestResults } = useAuth();
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      const testsData = await getAllTestResults();
      
      const processed = studentsData.map(student => {
        const sTests = testsData.filter(t => t.student_id === student.student_id).sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
        return {
          ...student,
          totalTests: sTests.length,
          testHistory: sTests,
          latestTest: sTests[0] || null,
          trend: sTests.length > 1 ? (sTests[0].score < sTests[1].score ? 'improving' : 'worsening') : 'stable'
        };
      }).filter(s => s.latestTest); // Hanya tampilkan yang pernah tes
      
      setStudents(processed);
    };
    loadData();
  }, [user, navigate, getAllStudents, getAllTestResults]);
